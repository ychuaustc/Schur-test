...
function [X, iterT, t] = SchurConjPreSolver(MSS, MSE, MWW, MWE, MEE, MEE_W, LW, LE, CS, CW, b, dsInd, dwInd, deInd, nV, numDecompose, epsSchur)

%%
tic;

%%	compute XE
Pb = Preconditioner(MWW, MWE, MEE_W, LW, LE, b);
l = length(Pb);
XE = zeros(l, 1);
Sx = SchurMultiply(MSS, MSE, MWW, MWE, MEE, XE, numDecompose);
Sx = Preconditioner(MWW, MWE, MEE_W, LW, LE, Sx);
r = Pb - Sx;
p = r;
rsold = r' * r;
%
...
for i = 1:l
    ...
    Sp = Preconditioner(MWW, MWE, MEE_W, LW, LE, Sp);
    alpha = rsold / (p' * Sp);
    XE = XE + alpha * p;
    r = r - alpha * Sp;
    rsnew = r' * r;
