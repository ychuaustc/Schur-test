...
%%          nV:         mesh size
%%  Output:
%%          FCotTheta:	cot(theta) of each triangle in Face
%%          MCotTheta:	matrix of cot(theta), MCotTheta_ij is the cotangent value of the angle towards half edge ij

function [FCotTheta, MCotTheta] = GeoMeshInfo(Face, FX, FY, nV)
...
E3X = F2X - F1X;
E3Y = F2Y - F1Y;
lE1 = sqrt(E1X .^ 2 + E1Y .^ 2);    %   length of edge toward vertex 1 in triangle
lE2 = sqrt(E2X .^ 2 + E2Y .^ 2);    %   length of edge toward vertex 2 in triangle
lE3 = sqrt(E3X .^ 2 + E3Y .^ 2);    %   length of edge toward vertex 3 in triangle
%
