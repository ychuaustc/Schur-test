function [X, iterT, t] = SchurConjPreSolver(MSS, MSE, MWW, MWE, MEE, MEE_W, LW, LE, CS, CW, b, solverMSS, dsInd, dwInd, deInd, nV, numDecompose, epsSchur)
%
...
rold = b - SX;
zold = Preconditioner(MWW, MWE, MEE_W, LW, LE, rold);
p = zold;
w = SchurMultiply(MSS, MSE, MWW, MWE, MEE, p, solverMSS, numDecompose);
alpha = (rold' * zold) / (p' * w);
XE = XE + alpha * p;
rnew = rold - alpha * w;
iterT = 1;
...
while norm(rnew, 2) > epsSchur
    znew = Preconditioner(MWW, MWE, MEE_W, LW, LE, rnew);
